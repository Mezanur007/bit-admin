...
export default function EditCaseStudy() {
  ...
  const p = useTranslations("editStudy");
  ...
  const removeSnap = (item) => {
    setStudy((prev) => ({
      ...prev,
      snaps: prev.snaps.filter((i) => i.id !== item.id),
    }));
    if (item.path) {
  ...
  useEffect(() => {
    if (id) {
      const foundEvent = caseStudies.find((s) => s.id === id);
      if (foundEvent) {
        setStudy({
          title: foundEvent.title || { en: "", ar: "" },
          slug: foundEvent.slug || "",
          service: foundEvent.service || { en: "", ar: "" },
          tags: foundEvent.tags || [],
          description: foundEvent.description || { en: "", ar: "" },
          banner: foundEvent.banner || null,
          snaps: foundEvent.snaps || [],
          videos: foundEvent.videos || [],
          applications: foundEvent.applications || [],
          features: foundEvent.features || [],
        });
      }
    }
  }, [id, caseStudies]);
